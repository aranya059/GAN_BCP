...
def self_train(args , snapshot_path):
    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            loss_dice = dice_loss(F.softmax(pre_a, dim=1), lab_a.unsqueeze(1))
            loss_ce = ce_loss(F.softmax(pre_a, dim=1), lab_a.type(torch.int64))
...
if __name__ == "__main__":
    ...
    # -- path to save models
    self_snapshot_path = "./model/Unet/ACDC_{}_{}_labeled/self_train".format(args.exp, args.labelnum)
    if not os.path.exists(self_snapshot_path):
        os.makedirs(self_snapshot_path)
    shutil.copy('../code/ACDC_unet_train.py', self_snapshot_path)
